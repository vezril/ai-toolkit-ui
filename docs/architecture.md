...
| **R-3: Unbounded concurrent child processes** — nothing in `lib/runs.ts` limits how many `promptfoo eval` processes can be in flight at once; every Run click spawns immediately | **High** (each run can invoke real, metered AI-CLI calls across up to 5 providers simultaneously — this is a cost/resource risk, not just a stability one; also no per-child timeout, so a hung provider CLI runs forever) | Medium (requires the user to click Run repeatedly before a prior run finishes — plausible during iterative EDD use) | ~~High~~ **Mitigated (v0.8.0, `add-run-guardrails`)** | Per-config in-progress guard, global concurrency cap (Settings, default 2), and a spawn timeout (default 15 min) that SIGTERM→SIGKILLs the child's **process group** (killing only the wrapper verifiably leaked the hung provider grandchild). Residual: guards are per-instance (R-2's single-instance assumption stands); process-group kill is POSIX-only. |
