...
| v0.9.0 | `add-skill-quick-fixes` | **Extends** `skill-builder` with verified quick-fixes for the mechanical finding class: quote-unparseable-frontmatter (offered only when the repair re-parses), sync-name-to-directory, fuzzy `[[link]]` repair at edit distance ≤ 2 (unwrap otherwise). Inline diff previews, content-hash-guarded apply, post-verified writes with automatic revert. No AI-assisted fixes — the app's no-direct-model-calls boundary stays intact. |
| v0.10.0 | `add-coach-guided-refinement` | **Extends** `eval-coach`: the four content-gap findings gain inline refinement forms in the builder's coach panel (answers become draft structure; Save remains the only write), and skill evals hint the placeholder refinement with trigger phrases extracted mechanically from the skill's description. Calibration findings stay advisory. No new write paths; no direct model calls. |
| v0.11.0 | `add-workflow-builder` | **Adds** `workflow-builder`: list + visualize Claude Code workflow scripts (declared phases, regex-detected composition edges, `~/.claude/workflows` sync status with one-click healing) and build sequential workflows visually — steps wire skills by name+description into generated, parse-validated, injection-safe Workflow-tool scripts written to both managed locations (fourth write location, narrowly scoped). Hand-written scripts are view-only (bounded fidelity, third application); the UI never executes workflows. **Extends** `app-settings` (workflows directory, third sandboxed root) and `sidebar-navigation` (Workflows entry). Parallel fan-out documented as follow-up. |
| v0.12.0 | `add-agents-tool` | **Adds** `agents-tool`: list/edit agent definitions (fourth configured root) with coached descriptions, tools/model frontmatter, full-fidelity round-trips including a no-op-save guard that preserves hand-written YAML style, structural validation with unknown-tool warnings, and deploy badges distinguishing `~/.claude/agents` symlinks from copies. **Extends** `app-settings` (agents directory) and `sidebar-navigation` (Agents entry). Prerequisite for `add-toolkit-versioning`. |
| v0.13.0 | `add-toolkit-versioning` | **Adds** `toolkit-versioning` (ADR-0011, the app's first git execution): scoped save-commits with auto-patch semver in per-directory `versions.json`, history + forward-only loss-free restore on all three component editors, and Ship (push to `toolkit-ui-ship` + gated PR) — never switching branches, never staging unrelated files, degrading to plain writes outside git. **Extends** `workflow-builder` (hand-written meta-surgery + raw-source editing; version-aware sync badges), `skill-builder`, and `agents-tool` (versioned saves + history panels). |
| v0.14.0 | `add-ollama-local-models` | **Adds** `local-model-providers`: a Local models group in the eval builder listing whatever a local Ollama instance serves (discovered live from `/api/tags`), each usable as free provider or judge; serializes to promptfoo-native `ollama:chat:<model>` with round-trip; non-default base URL injected into runs as `OLLAMA_BASE_URL`. Reachability-gated (not key-gated), calm not-detected state. Listing models is a localhost metadata call — inference stays in the promptfoo child, so the no-direct-inference boundary holds. **Extends** `app-settings` (Ollama base URL + reachability readout). HF-browse and OpenAI-compatible endpoints are documented follow-ups. |
| v0.15.0 | `add-docker-image` | **Adds** `containerization`: a self-contained Docker image that clones `vezril/claude-toolkit` into a persistent `/data` volume on boot, keeps builder evals in that volume, and is configured entirely by env (self-configuring defaults). Ships compose, a self-cloning entrypoint, and a tag-triggered Docker Hub `release.yml` (build-only without secrets). Bundles git + pinned promptfoo. Runs **without auth** — trusted-network only; this packages the existing single-user/no-auth stance into a container whose perimeter the operator owns (not multi-tenant hosting). **Extends** `local-model-providers` (Ollama base URL honors `OLLAMA_BASE_URL`). CLI runners remain host-only. |
