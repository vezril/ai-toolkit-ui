...
- **Release workflow**: the `release.yml` added in 0.15.0 startup-failed because the
  `secrets` context isn't permitted in a step `if:` expression; the push-vs-skip decision
  now flows through a step output computed from a secret-backed env var. This is the first
  tag that can actually publish the image to Docker Hub.
...
- Sidebar tool order is now Dashboard, New evaluation, Runs, Agents, Skills, Workflows
  (Agents moved ahead of Skills). No routes or behavior changed.
...
[0.15.1]: https://github.com/vezril/ai-toolkit-ui/compare/v0.15.0...v0.15.1
